...
get_match_indices <- function(ls_sp1, ls_sp2, match_values, i) {
    ...
    sp2 <- ls_sp2[i]
    pattern = paste0("(.*[", toupper(first_sp1), "|", tolower(first_sp1), "]", lower_sp1,
        ".*", sp2, ")")
    match_indices <- grep(pattern = pattern, ignore.case = F, x = match_values)
    ...
    if (length(name_match_indices) == 1) {
        ...
        if (length(name_match_indices > 1)) {
            for (j in name_match_indices) {
                ...
                alias_match_indices <- get_match_indices(ls_sp1, ls_sp2, HGNC$ALIAS_NAME[j],
                  i)
                if (length(alias_match_indices) == 1) {
                  return(j)
                } else {
                  prev_match_indices <- get_match_indices(ls_sp1, ls_sp2, HGNC$PREV_NAME[j],
                    i)
...
regex_iterate_match <- function(pattern) {
    ...
    while (nbr_rows != 1) {
        hgnc_df <- HGNC %>%
            filter(grepl(x = !!as.name(colnames[cnt]), pattern = pattern, perl = T,
                ignore.case = T))
...
clean_kinase_data <- function(HGNC, kinasedata, manual_map, cutoff = -10000, kinasefilter = "") {
    ...
    df <- kinasedata %>%
        # arrange(Kinase) %>%
    filter(Result >= cutoff, !grepl(pattern = "\\[", x = Kinase), !grepl(pattern = "^(?!.*\\/).*\\(.*\\)$",
        x = Kinase, perl = T)) %>%
        # !grepl(pattern = '/', x = Kinase), !grepl(pattern = '-', x = Kinase))
        # %>%
    mutate(No_Greek = case_when(grepl(pattern = "α", Kinase) ~ gsub("α", "A", Kinase),
        grepl(pattern = "β", Kinase) ~ gsub("β", "B", Kinase), grepl(pattern = "γ",
            Kinase) ~ gsub("γ", "G", Kinase), grepl(pattern = "δ", Kinase) ~ gsub("δ",
            "d", Kinase), grepl(pattern = "ᐃ", Kinase) ~ gsub("ᐃ", "D", Kinase),
        grepl(pattern = "ε", Kinase) ~ gsub("ε", "E", Kinase), grepl(pattern = "ζ",
            Kinase) ~ gsub("ζ", "Z", Kinase), grepl(pattern = "η", Kinase) ~ gsub("η",
            "H", Kinase), grepl(pattern = "θ", Kinase) ~ gsub("θ", "Q", Kinase),
        grepl(pattern = "ι", Kinase) ~ gsub("ι", "I", Kinase), TRUE ~ Kinase)) %>%
        mutate(No_Space = str_replace(No_Greek, " ", "_"), Just_Kinase = sub("_.*",
            "", No_Space), Just_Kinase = stringr::str_to_upper(Just_Kinase)) %>%
        separate(No_Space, into = c("sp1", "sp2", "sp3"), extra = "drop", remove = FALSE,
            fill = "right") %>%
        merge(reactive_data$ht, by.x = "Just_Kinase", by.y = "SYMBOL_UPPER", all.x = TRUE,
            all.y = FALSE) %>%
        arrange(desc(Result))
    ...
    for (i in seq(nrow(dtt))) {
        ...
        if (!is.na(x1)) {
            tmp.symbol <- HGNC %>%
                ...
                pull()
            df <- df %>%
                mutate(HGNC_ID = replace(HGNC_ID, Result == dtt$Result[i], x1), SYMBOL = replace(SYMBOL,
    ...
    if (loop_num > 0) {
        for (i in seq(1, loop_num)) {
            ...
            if (!is.na(idx)) {
                df <- df %>%
                  mutate(HGNC_ID = replace(HGNC_ID, Result == dt$Result[i], HGNC$HGNC_ID[idx]),
    ...
    SEC_HGNC <- HGNC %>%
        ...
        mutate(ALIAS_UPPER = stringr::str_to_upper(ALIAS_SYMBOL), PREV_UPPER = stringr::str_to_upper(PREV_SYMBOL))

    df_sym_nomatch <- df_sym_nomatch %>%
        mutate(HGNC_ID = NULL, SYMBOL = NULL)

    df_alias_match <- merge(df_sym_nomatch, SEC_HGNC, by.x = "Just_Kinase", by.y = "ALIAS_UPPER",
        all.x = T, all.y = F)
    ...
    df_prev_match <- merge(df_alias_nomatch, SEC_HGNC, by.x = "Just_Kinase", by.y = "PREV_UPPER",
        all.x = T, all.y = F)
    ...
    df_mnl_match <- merge(df_prev_nomatch, manual_map[, .(UPPER_KINASE, HGNC_ID,
        SYMBOL = GENE_SYMBOL)], by.x = "Just_Kinase", by.y = "UPPER_KINASE", all.x = TRUE,
        all.y = FALSE)
    ...
    kinome_data <- kinome_data %>%
        mutate(bin_25 = case_when(Result <= 25 ~ "25", Result <= 50 ~ "50", Result <=
            75 ~ "75", Result <= 90 ~ "90", Result > 90 ~ "100"), bin_10 = case_when(Result <=
            10 ~ "10", Result <= 20 ~ "20", Result <= 30 ~ "30", Result <= 40 ~ "40",
            Result <= 50 ~ "50", Result <= 60 ~ "60", Result <= 70 ~ "70", Result <=
                80 ~ "80", Result <= 90 ~ "90", Result > 90 ~ "100"), bin_25_range = case_when(bin_25 ==
            "25" ~ "0-25", bin_25 == "50" ~ "26-50", bin_25 == "75" ~ "51-75", bin_25 ==
            "90" ~ "76-90", bin_25 == "100" ~ "90-100"), bin_10_range = case_when(bin_10 ==
            "10" ~ "0-10", bin_10 == "20" ~ "11-20", bin_10 == "30" ~ "21-30", bin_10 ==
            "40" ~ "31-40", bin_10 == "50" ~ "41-50", bin_10 == "60" ~ "51-60", bin_10 ==
            "70" ~ "61-70", bin_10 == "80" ~ "71-80", bin_10 == "90" ~ "81-90", bin_10 ==
            "100" ~ "91-100")) %>%
    ...
    if (any(grepl(x = kinome_data$Just_Kinase, pattern = "c-mer", ignore.case = T))) {
        kinome_data <- kinome_data %>%
            mutate(SYMBOL = case_when(Just_Kinase == "C-MER" ~ "MERTK", TRUE ~ SYMBOL),
                HGNC_SYMBOL = SYMBOL)
...
clean_mutkinase_data <- function(HGNC, kinasedata, manual_map, cutoff = 50, kinasefilter = "") {
    ...
    kinasedata <- kinasedata %>%
        ...
        separate(KINASE, into = c("KINASE_SP1", "MUTANT_SP2", "XTRA_SP3"), remove = F,
            extra = "drop") %>%
        rename(Result = PCT_INHIBITION_AVG) %>%
        mutate(KINASE_NAME = case_when(nchar(KINASE_SP1) < 2 ~ stringr::str_to_upper(MUTANT_SP2),
            TRUE ~ KINASE_SP1), KINASE_NAME = case_when(grepl(x = KINASE_NAME, pattern = "aurora",
            ignore.case = T) ~ paste0(KINASE_NAME, ".{1,}", MUTANT_SP2), grepl(x = KINASE_NAME,
            pattern = "p38a", ignore.case = T) ~ "MAPK14", grepl(x = KINASE_NAME,
            pattern = "CK1epsilon", ignore.case = T) ~ "CSNK1E", TRUE ~ KINASE_NAME)) %>%
        merge(reactive_data$ht, by.x = "KINASE_NAME", by.y = "SYMBOL_UPPER", all.x = TRUE,
            all.y = FALSE) %>%
    ...
    kinasedata <- kinasedata %>%
        ...
        mutate(bin_25 = case_when(Result <= 25 ~ "25", Result <= 50 ~ "50", Result <=
            ...
            10 ~ "10", Result <= 20 ~ "20", Result <= 30 ~ "30", Result <= 40 ~ "40",
            Result <= 50 ~ "50", Result <= 60 ~ "60", Result <= 70 ~ "70", Result <=
                80 ~ "80", Result <= 90 ~ "90", Result > 90 ~ "100")) %>%
        arrange(desc(Result), SYMBOL) %>%
        filter(!grepl(x = KINASE, pattern = kinasefilter)) %>%
        mutate(HGNC_ID = case_when(is.na(HGNC_ID) ~ HGNC_ID_CHECK, TRUE ~ HGNC_ID),
