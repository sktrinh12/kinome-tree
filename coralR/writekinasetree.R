...
build.branch <- function(l) {
    branch = paste("<path id=\"b_x5F_", l["id.coral"], "\" fill=\"", l["branch.col"],
...
build.text <- function(l, labelselect) {
    ...
    label = paste("<a xlink:href=\"http://www.uniprot.org/uniprot/", l["id.uniprot"],
        "\" target=\"_blank\">", "<text id=\"t_x5F_", l["id.coral"], "\" ", "x=\"",
        l["text.x"], "\" ", "y=\"", trimws(l["text.y"]), "\" ", "font-weight=\"700\" ",
        " font-size=\"", l["text.size"], "px\" ", "fill=\"", l["text.col"], "\" ",
        "font-family=\"", l["text.font"], "\" ", "letter-spacing=", "\".035\"", ">",
        label, "</text>", "</a>", sep = "")
    return(label)
}
...
writekinasetree <- function(svginfo, destination, font, labelselect, groupcolor,
    ...
    outputlines = c(outputlines, paste("<text x=\"425\" y=\"", titley, "\" text-anchor=\"middle\" font-weight=\"700\" font-family=\"",
        font, "\"  font-size=\"", titlefontsize, "\">", svginfo$title, "</text>",
        sep = ""))
    ...
    outputlines = c(outputlines, unlist(lapply(svginfo$groups, build.group.labels,
        font = font, groupcolor = groupcolor)))
    outputlines = c(outputlines, "</g>")
