...
invert_maxmin <- function(vector.x) {
    ...
    # float or not
    if (any(sapply(rank_dsc, function(x) is_integer(x)) == F)) {
        missing_idx_dsc <- setdiff(seq(length(vector.x)), sapply(rank_dsc, function(x) as.integer(x)))
        missing_idx_asc <- setdiff(seq(length(vector.x)), sapply(rank_asc, function(x) as.integer(x)))
        # find the float numbers in the rank vectors
        asc_not_int <- which(sapply(rank_dsc, function(x) is_integer(x)) == F)
        dsc_not_int <- which(sapply(rank_asc, function(x) is_integer(x)) == F)
    ...
    rank_dsc <- sapply(rank_dsc, function(x) as.integer(x))
    rank_asc <- sapply(rank_asc, function(x) as.integer(x))
...
resizes.by.value <- function(df, resizedf, sizerange, controlledrange = FALSE, minvalue = 5,
    maxvalue = 60, showall = "show", sel_colm) {
    ...
    if (controlledrange == FALSE) {
        ...
        minvalue = 0
        maxvalue = 100
        if (max(resizedf[[sel_colm]]) > 100 && !grepl("ratio", sel_colm)) {
