...
connect_string <- paste0("jdbc:oracle:thin:@", ls_vars$host, ":", ls_vars$port, ":",
    ls_vars$sid)

print(connect_string)

# userdata schema connection
conn <- dbConnect(jdbcDriver, connect_string, ls_vars$username_userdata, ls_vars$password_userdata)
...
fetch_f_kdata <- function(compd_id, exp_id, tech_id = NA) {
    exp_id <- gsub(pattern = " \\(.*$", x = exp_id, replacement = "")
    query_str <- paste0(kin_panel_qstr, compd_id, "' AND EXPERIMENT_ID = '", exp_id,
        "'")
    if (!is.na(tech_id)) {
