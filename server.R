...
server <- function(input, output, session) {
    ...
    kinaseDataCleanedFiltered <- reactive({
        req(isTruthy(input$kinasefile) || isTruthy(input$submit))
        if (isTruthy(input$kinasefile)) {
            dt <- clean_kinase_data(HGNC, kinaseData(), manual_map, input$cutoff,
                input$kinasefilter)
        }
        if (isTruthy(input$submit)) {
            req(input$search_cmpd_ids, input$exp_id, input$tech_id)
    ...
    output$downloadInputData <- downloadHandler(filename = function() {
    ...
    output$downloadCleanedData <- downloadHandler(filename = function() {
        paste(input$kinasefile$name, "-cleaned-", format(Sys.time(), "%Y%m%d"), ".csv",
            sep = "")
    }, content = function(file) {
        write.csv(kinaseDataCleaned(), file, row.names = FALSE)
    })

    output$downloadCleanedFilteredData <- downloadHandler(filename = function() {
        paste(input$kinasefile$name, "-filtered-", format(Sys.time(), "%Y%m%d"),
            ".csv", sep = "")
    }, content = function(file) {
    ...
    insertUI(selector = "#treediv", where = "afterEnd", ui = source("coralR/renderTree.R",
        local = TRUE)$value)
    ...
    newdf <- reactive({
        ...
        if (dim(resizedf)[1] == 0) {
            createAlert(session, "alert", "resizedf_qcheck", title = "Error", content = "Cannot map to kinome tree; there was no HGNC match for those kinases",
                append = FALSE)
            return()
        } else {
            ...
            node_colours <- c(input$node_tgt_cpick, input$node_offtgt_cpick)

            label_colours <- c(input$text_tgt_cpick, input$text_offtgt_cpick)
            ...
            if (nrow(resizedf) > 0) {
                # sizerange = c(1, 60)
                radii_and_mapping = resizes.by.value(df = tempdf, resizedf = resizedf,
                  sizerange = c(input$nodesizemin, input$nodesizemax), controlledrange = FALSE,
                  minvalue = 0, maxvalue = 100, showall = "hide", sel_colm = sel_colm)
            }
            ...
            if (grepl(x = sel_colm, pattern = "bin")) {
                resizedf <- radii_and_mapping[[3]] %>%
                  mutate(radii = case_when(!!as.name(sel_colm) <= 60 & !!as.name(sel_colm) >
                    0 ~ radii * 0.04, !!as.name(sel_colm) <= 70 & !!as.name(sel_colm) >
                    60 ~ radii * 0.18, !!as.name(sel_colm) <= 80 & !!as.name(sel_colm) >
                    70 ~ radii * 0.3, !!as.name(sel_colm) <= 90 & !!as.name(sel_colm) >
                    80 ~ radii * 0.5, TRUE ~ radii))
            ...
            # font size
            tempdf <- tempdf %>%
                mutate(node.selected = ifelse(id.coral %in% resizedf$id.coral, 1,
                  -1), node.radius = radii_and_mapping[[1]], node.val.radius = radii_and_mapping[[2]],
                  text.size = ifelse(node.selected == 1, input$nodelabelfontsize,
                    0), node.opacity = input$nodeopacity/100, branch.col = BG_col1)


            # set node colours
            tempdf <- tempdf %>%
                mutate(node.col = case_when(id.HGNC %in% hgnc_node_selected ~ node_colours[1],
                  TRUE ~ node_colours[2]), text.col = ifelse(node.col == node_colours[1],
                  input$text_tgt_cpick, input$text_offtgt_cpick))
            ...
            tdf_node_size <- tempdf %>%
                ...
                distinct(!!as.name(ifelse(grepl(x = sel_colm, pattern = "ratio"),
                  paste0(sel_colm, "_range"), sel_colm)), .keep_all = T) %>%
                mutate(row_index = row_number(), cumsum_radius = cumsum(2 * node.radius +
                  buffer), y_pos = ifelse(row_index == 1, y + node.radius, lag(cumsum_radius) +
                  node.radius + y), cy_pos = y_pos, x_pos = 140.9432 - (1.25 * buffer +
                  max(node.radius)), cx_pos = 160.9432, radius = node.radius, label = !!as.name(paste0(sel_colm,
                  "_range")), colours = BG_col1, font_size = 9) %>%
    ...
    output$plot1 <- renderSvgPanZoom({
        ...
        # Write SVG file
        writekinasetree(svginfo, destination = svgoutfile, font = "Arial", labelselect = "HGNC",
            groupcolor = "#000000", titley = input$titley, titlefontsize = input$titlefontsize)

        # Render SVG
        svgPanZoom(svgoutfile, viewBox = F, controlIconsEnabled = F)
    })

    output$downloadtree <- downloadHandler(filename <- function(file) {
        "CORAL.tree.svg"
    }, content <- function(file) {
    ...
    observeEvent(input$submit, {
        ...
        reactive_data$df_kd <- fetch_f_kdata(input$search_cmpd_ids, input$exp_id,
            input$tech_id)
        output$kinasetable <- renderDataTable(reactive_data$df_kd, options = list(pageLength = 25))

        # cleaned data
        reactive_data$df_c <- clean_kinase_data(HGNC, reactive_data$df_kd, manual_map)

        # cleaned & filtered data
        reactive_data$df_f <- clean_kinase_data(HGNC, reactive_data$df_kd, manual_map,
            input$cutoff, input$kinasefilter)

        # update the node selection for target nodes
        updateSelectInput(session, "node_colours_tgt", choices = reactive_data$df_f$HGNC_SYMBOL,
            selected = NULL)


        # update the node selection for off target nodes
        updateSelectizeInput(session, "node_colours_offtgt", choices = reactive_data$df_f$HGNC_SYMBOL,
            selected = NULL)

    })

    # poll the unique values every 1x10^6 milliseconds (~17 mins)
    current_cmpd_ids <- reactivePoll(1e+06, session, checkFunc = fetch_len_cmpd_ids,
        valueFunc = fetch_unq_cmpd_ids)

    # update the search compound id input box with distinct values
    observe({
        updateSelectizeInput(session, "search_cmpd_ids", choices = current_cmpd_ids(),
    ...
    observe({
        ...
        updateSelectizeInput(session, "search_cmpd_ids", choices = current_cmpd_ids(),
            selected = param_query[["search_cmpd_ids"]], server = TRUE)
    })
    ...
    output$exp_id_output <- renderUI({
        ...
        if (length(mdata_result) > 0) {
            meta_data <- sapply(seq(1:length(exp_id_list)), function(i) {
                paste0(mdata_result[, i]$EXPERIMENT_ID, " (", mdata_result[, i]$PROPERTY_VALUE,
                  ")", " [", mdata_result[, i]$CONC, " nM]", " [", mdata_result[,
                    i]$ORDER_NUM, "]")
            })
            # dropdown menu that allows for selection of experimental id
            selectInput(inputId = "exp_id", label = "Experiment ID (CRO) [Conc] [Order]",
                choices = meta_data, selected = NULL, multiple = FALSE)
        } else {
            return(NULL)
        }
    })
    ...
    output$tech_output <- renderUI({
        ...
        if (length(tech_list) > 0) {
            # dropdown menu that allows for selection of experimental id
            selectInput(inputId = "tech_id", label = "Technology", choices = tech_list,
    ...
    observeEvent(input$click_polar, {
        ...
        dt <- clean_mutkinase_data(HGNC, reactive_data$df_kd, manual_map, input$cutoff,
            input$kinasefilter)

        if (is.null(dt)) {
            createAlert(session, "alert", "dt_polar_qcheck", title = "Error", content = "Could not fetch a valid dataset",
                append = FALSE)
            return()
        }
        ...
        if (dim(dt)[1] == 0) {
            createAlert(session, "alert", "dt_polar_qcheck", title = "Error", content = "No mutant kinase data to plot",
                append = FALSE)
            return()
        }
        ...
        reactive_data$polarplot <- ggplot2::ggplot(dt, ggplot2::aes(x = as.factor(INDEX),
            y = Result, fill = HGNC_SYMBOL)) + ggplot2::geom_bar(ggplot2::aes(x = as.factor(INDEX),
            y = Result), stat = "identity", alpha = 0.7) + ggplot2::ylim(-20, 122) +
            ggplot2::theme_minimal() + ggplot2::theme(axis.text = ggplot2::element_blank(),
            axis.title = ggplot2::element_blank(), panel.grid = ggplot2::element_blank()  #,
) + ggplot2::coord_polar() + ggplot2::geom_text(ggplot2::aes(x = INDEX,
            y = Result + 4, label = CRO_Kinase, hjust = hjust), color = "black",
            fontface = "bold", alpha = 0.9, size = 2.5, angle = dt$angle) + ggplot2::geom_text(ggplot2::aes(x = INDEX,
            y = Result - 20, label = round(Result, 1), hjust = hjust), color = "black",
            fontface = "bold", alpha = 0.6, size = 3, angle = dt$angle) + ggplot2::ggtitle(paste("Mutant Pct Inb Polar Histogram -",
    ...
    observeEvent(input$click_lolli, {
        ...
        if (is.null(dt)) {
            createAlert(session, "alert", "dt_lolli_qcheck", title = "Error", content = "Filters were too strict, no data to display",
                append = FALSE)
            return()
        }
        ...
        if (dim(dt)[1] == 0) {
            createAlert(session, "alert", "dt_lolli_qcheck", title = "Error", content = "No mutant kinase data to plot",
                append = FALSE)
            return()
        }
        ...
        reactive_data$lolliplot <- ggplot2::ggplot(dt, ggplot2::aes(x = CRO_Kinase,
            y = Result)) + ggplot2::geom_segment(ggplot2::aes(x = CRO_Kinase, xend = CRO_Kinase,
            y = 0, yend = Result, colour = factor(HGNC_SYMBOL)), size = 0.7) + ggplot2::geom_point(ggplot2::aes(colour = factor(HGNC_SYMBOL)),
            size = 2) + ggplot2::geom_text(ggplot2::aes(x = CRO_Kinase, y = Result,
            label = round(Result, 1)), nudge_y = -5, size = 3) + colours + ggplot2::coord_flip() +
    ...
    output$downloadlolliplot <- downloadHandler(filename <- function(file) {
        paste("lolliplot_", input$search_cmpd_ids, ".png")
    }, content <- function(file) {
        ggplot2::ggsave(file, reactive_data$lolliplot)
    })

    output$downloadpolarplot <- downloadHandler(filename <- function(file) {
